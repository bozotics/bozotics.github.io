...
Hence, having a low CG is crucial in ensuring the robot can accelerate quickly **in a reliable manner**, allowing for more **consistent** motion across the field. Some low CG features of our robot:
...
Here is a video of our robot spinning on the spot with the motors at roughly 40% power. There is some oscillations in its movement due to the weight imbalance from the solenoid, but its overall position does not drift much, which shows it has rather good weight distribution.
...
INSERT GALLERY HERE (photo of carbon fibre, photo of 3d print in progress 20% infill, motor on weighing scale)
...
Our robot is built to be very stable since there are 8-point connections throughout the robot's layers using strong hex brass standoffs, be it between the bottom 3 main layers or up to the mirror plate's support. The motors are wedged between 2 layer plates, ensuring they do not flex inwards under the robot's weight.

INSERT GALLERY OF STURDINESS (8 point support in bottom 3 layer, 8 point support in mirror, motor between 2 layer)
