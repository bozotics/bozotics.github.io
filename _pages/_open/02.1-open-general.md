...
A more intuitive way to look at this would be from the free body diagram of a braking robot. Given a robot with CG at point C<sub>1</sub> vs C<sub>2</sub>, it can be seen that W<sub>2</sub> is closer to the "lifted" side, giving the wheels on that side of the robot "more traction" than if it were W<sub>1</sub>.
