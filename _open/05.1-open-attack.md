...
In the graph below, the red line represents the equation above with arbitrary constants for *a*, *b* and *c*. By increasing *a*, the graph transforms into the blue and black lines, while increasing *b* transforms it into the green and purple lines. As shown, by modifying both constants, an ideal response curve can be generated, which would allow the robot to quickly increase its offset angle as it gets closer to the ball, while preventing its arc path from being too large.
