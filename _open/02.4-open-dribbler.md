...
Similarly, if the value of θ is bigger, *F<sub>N:ball on roller</sub>*, would be smaller, since the magnitude of the component parallel to *F<sub>N:ball on roller</sub>* (red arrow, *x<sub>1</sub>*) of the downwards force at the end of the lever (blue arrow) is smaller. This means that a greater proportion of *F<sub>S:ground on ball</sub>* is in the direction parallel to *F<sub>N:ball on roller</sub>* (red arrow, *x<sub>2</sub>*), which might result in better ball to roller adhesion (think about it intuitively as the ball is rolling "into the roller" more when it is lower).

However, the total magnitude of *F<sub>S:ground on ball</sub>* itself decreases. Hence, the actual *hold* on the ball decreases, while the amount of torque needed to spin it decreases.

>Simplified: The higher the roller is on the ball, the stronger it can hold onto the ball, but the harder it is for the ball to spin.
...
{% include gallery id="gallery2" caption="Rollers vs Bar" %}
...
{% include gallery id="gallery3" caption="Low vs High pivot point" %}

However, a really big brain fart during the designing process was how we mistakenly thought the height of the pivot should be compared to the centre of the dribbler roller instead of the contact point. This resulted in our pivot point actually being slightly higher than the contact point. Luckily, the height difference was too small to make a significant impact, but this will definitely be changed ([AFI](#sike)).

PIC OF DRIBBLER HIGHER POINT
...
DIAGRAMS SHOWING CLOCKWISE MOMENT WITH FRONT OF CG

So, the pivot point must be behind the centre of gravity, but how far? The further the pivot point is from the centre of gravity, the larger the downwards force acting on the ball from the rollers. Hence, a larger minimum upwards force is required to tilt the dribbler up.

2 DIAGRAMS SHOWING MOMENTS/FBD

Hence, the pivot point should be located just slightly behind the centre of gravity if we want the dribbler to be lifted up more easily. For design convenience, we simply chose the bottom inner leg of the motor mount as the pivot point. Thus, the circular aluminium rods used to secure the motor in place doubled up as axles for the dribbler's fulcrum. A slightly oversized hole held these aluminium rods in place, leaving just enough tolerance that the dribbler can pivot smoothly but not too much that it starts wiggling about.
...
3. The spring was rather soft to begin with, so the recoil force was small enough for it to not affect the system too much.
