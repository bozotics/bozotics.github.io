...
There are 3 main parameters to tweak in our catoptric system.

#### Mirror curvature

The larger the angle of elevation of the mirror profile, by law of reflection (angle of incidence = angle of reflection), the larger the FOV of the system.
This results in smaller objects and greater distortion.

#### Mirror-Camera Distance

The larger the distance between the mirror and camera, assuming the camera's height stays constant, the smaller the amount of the camera's FOV being covered by the mirror.
This results in a smaller image, smaller FOV of the system, but less distortion

DIAGRAMS + COMPARED RENDER

#### Camera Height

The larger the distance between the camera and the ground, asssuming the mirror-camera distance remains constant, the larger the FOV of the system.

#### Ideal Mirror

### Design

In order to view the whole field with only one camera, a curved mirror is used to provide 360 degree vision for the robot. However, 
